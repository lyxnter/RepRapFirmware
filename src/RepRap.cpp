	...
	/* Temperatures */
	{
		...
		if (bedHeater != -1)
		{
			response->catf("\"bed\":{\"current\":%.1f,\"active\":%.1f,\"standby\":%.1f,\"state\":%d,\"heater\":%d},",
				...
				(double)heat->GetStandbyTemperature(bedHeater), heat->GetStatus(bedHeater), bedHeater);
		}
		...
		if (chamberHeater != -1)
		{
			response->catf("\"chamber\":{\"current\":%.1f,\"active\":%.1f,\"standby\":%.1f,\"state\":%d,\"heater\":%d},",
				...
				(double)heat->GetStandbyTemperature(chamberHeater), heat->GetStatus(chamberHeater), chamberHeater);
		}
		...
		if (cabinetHeater != -1)
		{
			response->catf("\"cabinet\":{\"current\":%.1f,\"active\":%.1f,\"standby\":%.1f,\"state\":%d,\"heater\":%d},",
				...
				(double)heat->GetStandbyTemperature(cabinetHeater), heat->GetStatus(cabinetHeater), cabinetHeater);
		}
...
OutputBuffer *RepRap::GetIPResponse()
{
	// We need some resources to return a valid config response...
	OutputBuffer *response;
	if (!OutputBuffer::Allocate(response))
	...
	response->copy("{\"ipAddress\":");
	response->catf("\"%s\", \"RRF_v\":", IP4String(platform->GetIPAddress()).c_str());
	response->catf("\"Machine version : %s\", \"door\": \"%d\", \"leds\": \"%d\", \"panelPin\": \"%s\" }", VERSION, (IoPort::ReadPin(platform->inputDoorState)), platform->GetLEDs(), reprap.panelPin.c_str());
