...
bool GCodes::HandleMcode(GCodeBuffer& gb, const StringRef& reply)
{
	...
	case 24: // Print/resume-printing the selected file
		/*
		if (digitalRead(platform.inputDoorState)) { // lynxmod
			reply.copy("Merci de fermer la porte avant de reprendre l'eventuelle impression !\n");
			result = GCodeResult::error;
		}
		*/
		if (IsPausing() || IsResuming() /*|| !digitalRead(platform.inputDoorState)*/) // lynxmod
		{
	...
	/*case 600:
		gb.SetState(GCodeState::probingServoOffset);
		InitLynxStateMachine();
		break;
	 */
	...
	case 969: // Communication with LynxMod
		if (gb.Seen('S')) { // Paramètre S
			unsigned int val = gb.GetFValue();
			platform.ComP = -1;
			if (gb.Seen('P')) { // Paramètre P
				unsigned int pval = gb.GetFValue();
				platform.ComP = pval;
			}
			if (gb.Seen('R')) { // Paramètre R
				unsigned int rval = gb.GetFValue();
				platform.ComCol[0] = rval;
			}
			if (gb.Seen('V')) { // Paramètre V
				unsigned int vval = gb.GetFValue();
				platform.ComCol[1] = vval;
			}
			if (gb.Seen('B')) { // Paramètre B
				unsigned int bval = gb.GetFValue();
				platform.ComCol[2] = bval;
			}
			if (gb.Seen('W')) { // Paramètre W
				unsigned int wval = gb.GetFValue();
				platform.ComCol[3] = wval;
			}
			if (gb.Seen('L')) { // Paramètre L
				unsigned int lval = gb.GetFValue();
				platform.ComCol[4] = lval;
			}
			platform.ComC = 969;
			platform.Com = val;
			platform.ComDate = millis();
		}
		break;
