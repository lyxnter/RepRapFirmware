...
bool GCodes::HandleMcode(GCodeBuffer& gb, const StringRef& reply)
{
	...
	case 32: // Select file and start SD print
		...
		}
		{
			...
			{
#if SUPPORT_LYNXMOD
					if (lynxMod.EnableSafeties && digitalRead(INPUT_DOOR_STATE)) { // lynxmod
						reply.copy("Merci de fermer la porte avant de lancer l'impression\n");
						//result = GCodeResult::error;
					}
#endif
					if (QueueFileToPrint(filename.c_str(), reply)
#if SUPPORT_LYNXMOD
							&& (!lynxMod.EnableSafeties || !digitalRead(INPUT_DOOR_STATE))
#endif
					)
				{
...
#if SUPPORT_LYNXMOD
			if (digitalRead(INPUT_DOOR_STATE) && lynxMod.EnableSafeties) { // lynxmod
				reply.copy("Merci de fermer la porte avant de reprendre l'impression\n");
				result = GCodeResult::error;
			}
#endif
	...
	case 675:
		...
		break;
			...
			/* LYNXMOD */
#if SUPPORT_LYNXMOD
		case 680: // Set pin code for the Pannel Due (not in use anymore but kept)
		{
			String<5> oldPanelPin;
			String<5> newPanelPin;
			bool seen = false;
			// Verification du password en P
			gb.TryGetPossiblyQuotedString('P', oldPanelPin.GetRef(), seen);
			if (seen) {
				if (reprap.CheckPanelPin(oldPanelPin.c_str())) {
					// Match
					seen = false;
					reprap.GetPlatform().MessageF(MessageType::HttpMessage, "Panel pin match\n");
					gb.TryGetPossiblyQuotedString('S', newPanelPin.GetRef(), seen);
					if (seen) {
						bool correct = true;
						bool enabled = false;
						for (unsigned char u=0;u<4;u++) {
							if (newPanelPin.c_str()[u] < 48 || newPanelPin.c_str()[u] > 57) {
								correct = false;
							} else {
								enabled = enabled?enabled:((newPanelPin.c_str()[u]-'0')!=0);
							}
						}
						if (correct) {
							newPanelPin[4] = 0;
							if (enabled)
								reprap.GetPlatform().MessageF(MessageType::HttpMessage, "New panel pin set (%s)\n", newPanelPin.c_str());
							else
								reprap.GetPlatform().MessageF(MessageType::HttpMessage, "Panel pin disabled\n");
							reprap.SetPanelPin(newPanelPin.c_str());
						}
					}
				}
			}
	...
	case 929: // Start/stop event logging
		...
		case 969: // Communication with LynxMod:lights
			...
			if (gb.Seen('S')) { // Param\E8tre S
				...
				lynxMod.ComP = -1;
				if (gb.Seen('P')) { // Parametre P
					unsigned int pval = gb.GetFValue();
					lynxMod.ComP = pval;
				}
				for (int i = 0; i < 5; i++)
					lynxMod.ComCol[i] = -1;
				if (gb.Seen('R')) { // Parametre R
					unsigned int rval = gb.GetFValue();
					lynxMod.ComCol[0] = rval;
					lynxMod.ComP = (lynxMod.ComP >= 0 ? lynxMod.ComP : -2);
				}
				if (gb.Seen('V')) { // Parametre V
					unsigned int vval = gb.GetFValue();
					lynxMod.ComCol[1] = vval;
					lynxMod.ComP = (lynxMod.ComP >= 0 ? lynxMod.ComP : -2);
				}
				if (gb.Seen('B')) { // Parametre B
					unsigned int bval = gb.GetFValue();
					lynxMod.ComCol[2] = bval;
					lynxMod.ComP = (lynxMod.ComP >= 0 ? lynxMod.ComP : -2);
				}
				if (gb.Seen('W')) { // Parametre W
					unsigned int wval = gb.GetFValue();
					lynxMod.ComCol[3] = wval;
					lynxMod.ComP = (lynxMod.ComP >= 0 ? lynxMod.ComP : -2);
				}
				if (gb.Seen('L')) { // Parametre L
					unsigned int lval = gb.GetFValue();
					lynxMod.ComCol[4] = lval;
					lynxMod.ComP = (lynxMod.ComP >= 0 ? lynxMod.ComP : -2);
				}
				lynxMod.ComC = 969;
				lynxMod.Com = val;
